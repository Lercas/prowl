...
Scan every repository under a GitHub org or user, a GitLab group, or a Bitbucket workspace in one command. Prowl lists the repositories via the platform's REST API, then clones and scans them in parallel. Each repository is cloned into a temporary directory, scanned, and its checkout is removed as it finishes — nothing is left on disk.
