...
The `context.line` is load-bearing: the 44 features are extracted from the *surrounding line*, not just the bare value, so a row's context must be the real line the candidate appeared on or the retrain learns from a fiction. This is exactly why `--show-secrets` now emits the full unredacted value **together with the surrounding line context** for authorized triage (see [Configuration](Configuration.md)). A finding triaged that way maps straight onto a feedback row — `value` and `context.line` come verbatim from the scan — so the labels you feed back are *faithful*: the model retrains on the same line text it will see in production, not a reconstructed approximation.
...
- Both **drop** a candidate the model judges not-a-secret — score below `--ml-threshold` (default **0.3**) — **except** a checksum-proven hit, which is never dropped.
