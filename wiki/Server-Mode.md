...
curl -s localhost:8080/scan \
  ...
  -d '{"source":"code","path":"config/prod.env","content":"AKIA2E0A8F3B244C9986"}'
```
...
| `source` | string | Logical source label. Defaults to `code` if empty. Typically one of `code`, `jira`, `confluence`, `slack`, `log` (any string is accepted and echoed back). |
...
| `truncated` | Items whose content was truncated before scanning. |
